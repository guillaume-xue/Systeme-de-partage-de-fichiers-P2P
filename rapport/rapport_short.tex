...
Un mécanisme de keep-alive envoie périodiquement des pings (tous les 30s) au serveur central et à tous les pairs connectés pour maintenir les mappings NAT et détecter les pairs inactifs.
...
Le downloader utilise 3 workers : SenderLoop (envoie requêtes), ResponseLoop (traite réponses), MonitorLoop (gère timeouts). La fenêtre s'ajuste dynamiquement (1-100 paquets) en fonction des performances réseau.
...
\begin{enumerate}
    ...
    \item Les deux exécutent du hole punching simultané avec backoff exponentiel (1s, 2s, 4s)
